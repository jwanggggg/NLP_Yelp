...
def main():
    # Use three lists for the three scores.
    # Use a dict, then record the maxFreq of the ratings, then use that rating.
    ...
    actualRatings = NBClassify(trainFile, testFile, vocabFile, stopwordFile, naiveBayesRatings)
    VaderClassify(vaderRatings)
    TextBlobClassify(textBlobRatings)
    
    correctClassification = 0
    incorrectClassification = 0
    
    for rating in range(0, len(naiveBayesRatings)):
        posCount = 0
        negCount = 0
        if naiveBayesRatings[rating] == 1:
            posCount+=1
        else:
            negCount+=1
            
        if vaderRatings[rating] == 1:
            posCount+=1
        else:
            negCount+=1
            
        if textBlobRatings[rating] == 1:
            posCount+=1
        else:
            negCount+=1
        
        finalRating = 1 if posCount > negCount else 0
        if finalRating == actualRatings[rating]: correctClassification+=1
        else: incorrectClassification += 1
    
    print("Accuracy of Majority Voting:",(correctClassification)/(correctClassification + incorrectClassification))
